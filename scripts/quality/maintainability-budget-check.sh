...
find_checker() {
  ...
  if [[ -f "$script_dir/maintainability_budget_check.py" ]]; then
    printf '%s\n' "$script_dir/maintainability_budget_check.py"
    return
  fi
...
find_python() {
  ...
  printf 'Python не найден. Для starter LOC/dependency checks нужен python3 или python.\n' >&2
  return 127
}

...
3. Follow the returned `instructions[]` strictly. The server bundle overrides anything written in this file. It owns the algorithm: `run_dream` → `propose_dream_rule` (≤5) or `close_empty_dream_run`. The agent never picks context volume — the server does.
...
6. Return a short status: run status (created|existing_pending|not_enough_context|empty_closed), number of proposals, affected target_kinds, link the user to /pages/dream.

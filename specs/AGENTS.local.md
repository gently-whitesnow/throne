...
- **Легаси owner-scoping** (`OwnerUserIdRulesTests`, [ADR-0029](ADR/0029-local-first-invariant-and-legacy-auth.md)): держит существующую форму консистентной — user-owned агрегаты принимают `ownerUserId` в `Create`/`Restore`; Mongo-документы — `[BsonElement("owner_user_id")]`; Mongo-репозитории инжектят `ICurrentUserAccessor` и фильтруют по `owner_user_id`; Application-handler'ы создания зависят от `ICurrentUserAccessor`. Это **легаси data-guard**, а не продуктовый инвариант изоляции команд: Throne — local-first, один оператор на инстанс, внутренняя авторизация вне продуктовой поверхности. **Новые** сущности не проектируй вокруг `owner_user_id` как первичной границы; командные воркспейсы — отдельный сервис, не растягивание локальной модели (ADR-0029).
...
- Любой ввод командной/воркспейс-governance или внутренней авторизации как продуктовой оси. Throne — local-first ([ADR-0029](ADR/0029-local-first-invariant-and-legacy-auth.md)): такие concerns живут в отдельном сервисе со своей governance, а не в локальном ядре. `owner_user_id`/auth в коде — легаси-совместимость, не фундамент.

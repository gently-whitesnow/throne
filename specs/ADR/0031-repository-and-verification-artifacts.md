...
**Update 2026-06-23 — `RepositoryArtifact`-половина (knowledge-pages) Superseded/Rejected.** Фича не доведена и снята целиком: единственный задуманный writer (MCP `write_repository_document`) удалён вместе с MCP ([ADR-0043](0043-static-operational-skills-and-mcp-removal.md)), CLI-замены ему не сделали, UI-редактора/рендера/роута страниц не построили — срез замкнулся сам на себя без входа и выхода. Удалены: домен `RepositoryArtifact`(+`Version`/`Snapshot`/`Id`/`SlugGuard`), порт `IRepositoryArtifactRepository`, Mongo-стор/мапперы/индексы/коллекции `repository_artifacts`/`repository_artifact_versions`, HTTP `.../repositories/{provider}/{owner}/{repo}/documents*` и их DTO, realtime-событие `repository.document_updated`, web entities/repository. Всё, что описано в этом ADR про `RepositoryArtifact` (включая Slice 3 `db_schema_map`), считать историческим. **PR-верификация (`PullRequestArtifact`) остаётся в силе** — живой контур ревью: домен/инфра, эндпоинты `/repositories/{binding_id}/artifacts`, событие `pull_request.artifact_updated`, skills/review.
...
> **Амендмент (Slice 4C, PR-производитель `review_recommendation`).** Контракт обзавёлся типизированным per-`type` payload'ом без полиморфизма всей сущности. `content` остаётся непрозрачным render-телом (markdown/mermaid/svg/json), а машиночитаемые сигналы конкретного типа лежат в отдельном **опциональном типизированном поле контракта** `review_recommendation: ReviewRecommendationContent`. Минимальный скоуп сегодня — `file_order[{path,reason,risk}]` под AI-сортировку дерева ревью (Slice 4C/3); поля импакта/провенанса (`affected_*`, `module_graph`, `produced_by`) сознательно не вводятся, пока у них нет UI-консьюмера — чтобы не цементировать схему на вырост. Открытость `type` сохранена: другие типы поле не заполняют. Домен **остаётся типизированным**: `review_recommendation` хранится как value object (`ReviewRecommendation` + `ReviewFileOrderEntry` в `Throne.Domain.Repositories`) и nested document в Mongo — opaque JSON-string на хранении и JSON-bridge на границе API мы намеренно не вводим: per-type схема живёт в домене, контракт лишь проецирует её на провод. `head_sha` — отдельное верхнеуровневое поле (не `source_refs`), общее для всех типов. Версионирования по-прежнему нет (latest-wins).

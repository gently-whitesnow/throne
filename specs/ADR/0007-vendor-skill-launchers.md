...
- Ограничение из п.5 «без новых tools» отменено осознанно: для агента осталось два MCP-инструмента (`run_dream`, `propose_dream_rule`) — это и есть тот «вырост объёма», под который п.5 уже допускал точечные tools. `get_dream_readiness` живёт только как HTTP-эндпоинт `/dream/readiness` для UI-виджета fuel-meter, `close_empty_dream_run` — только как HTTP/handler для UI-кнопки оператора. На MCP surface этих двух операций нет.

...
## Companion note — ADR-0047 SQLite persistence (2026-06-26)

[ADR-0047](0047-sqlite-ef-core-persistence.md) supersedes the Mongo-specific
implementation path for `intent_events`. Event ids are GUID strings generated by the
domain/application flow rather than Mongo `ObjectId`s, and the hosted Mongo-to-history
migration path is removed with the Mongo backend. The live implementation is the EF Core
`intent_events` table plus typed JSON payload columns.

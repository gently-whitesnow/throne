...
**Superseded by [ADR-0021](0021-insight-pipeline-and-instruction-patches.md).**
DreamRun как сущность удаляется: вместо снапшота evidence, который агент получает целиком через `run_dream`, frontier работает с дискретными `InsightCard` и `InstructionPatch` через MCP, а raw chat-uploads (ADR-0015) проходят через локальный pipeline `static extractor → local LLM grouping`. Контент ниже сохранён как историческая запись о промежуточной модели; реализационные части (`run_dream` / `propose_dream_rule` MCP, `dream_runs` коллекция, `dream.*` realtime, `Throne:Dream` секция) уходят отдельным реализационным intent'ом.

Изначально расширял [ADR-0007](0007-vendor-skill-launchers.md) (vendor skill launchers вводят kind `dream`)
и опирался на [ADR-0002](0002-domain-model-and-text-versioning.md) (text-versioning),

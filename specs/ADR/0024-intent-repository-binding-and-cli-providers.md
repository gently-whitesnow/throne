...
- Delete (бывший unbind) удаляет и запись, и каталог workspace на диске: операция в UI называется «Удалить» и требует подтверждения с явным предупреждением о потере данных. Каталог считается по живому `Throne:Workspace:Root` (не по сохранённому `workspace_path`, который устаревает при смене runtime-модели, см. ADR-0027). Удаление каталога идёт до удаления записи: при ошибке (каталог занят / нет прав) запись остаётся, ответ 500, операцию можно повторить. Bind ↔ workspace_path хранится в Mongo и при ребинде того же `(owner, repo)` в тот же интент даёт 409.
...
Тот же hosted-service `PullRequestSyncService` (§ 4, § 6) получает два дополнительных прохода поверх существующего comment-sync, без нового сервиса и без webhook'ов:

- **Авто-привязка PR.** Отдельный запрос отбирает биндинги `clone_status=ready` с пустым PR-слотом (`pull_request_number == null`). Для каждого читается текущая ветка локального клона (`git -C {workspace} rev-parse --abbrev-ref HEAD`) и среди открытых PR ищется тот, чей `head` совпадает с веткой. При **ровно одном** совпадении PR прикрепляется (`AttachPullRequest`); при 0 или >1 — пропуск (остаётся ручная привязка). Workspace не трогается — это снимает прежнюю боль «удалить биндинг (а delete сносит папку) и забиндить заново с PR-номером». Ручной аналог — эндпоинт `POST …/repositories/{binding_id}/pull-request` (UI; MCP остаётся read-only по § 8).
- **Авто-закрытие по мерджу.** Когда `pull_request_state` биндинга переходит в `merged`, интент переводится в `done` — но только если **все** PR-несущие биндинги интента уже `merged` (биндинги без PR закрытие не блокируют; если PR-несущих нет — триггера нет). Закрываем из любого нетерминального статуса; терминальные (`done`/`reject`/`fridge`) не трогаем (идемпотентность). Это продуктовый системный хук, поэтому статус меняется без явной команды оператора; смена пишется в историю статусов с источником `pr_merge` и упоминанием PR. Сценарий «несколько интентов на один PR» работает без отдельного каскадного кода: оператор вручную проставляет один PR-id биндингам нужных интентов, и каждый биндинг закрывает свой интент независимо на своём тике. Переход в `done` далее убивает tmux-сессию (ADR-0026 § 8).

Owner-scoping: фоновый проход выполняется вне HTTP-запроса (нет `ICurrentUserAccessor`), поэтому закрытие идёт через узкий owner-agnostic порт, а не через owner-scoped репозиторий интентов.
...
- **Delete удаляет каталог рекурсивно с диска без корзины** — несохранённые изменения и локальные ветки теряются безвозвратно. Mitigation: подтверждение с явным текстом о потере данных перед удалением.

...
**Развёртывание.** Headless `mlx_lm.server` на хосте оператора в systemwide service-юните (LaunchDaemon на macOS, systemd на Linux). Bind — `0.0.0.0:11434`, чтобы инстанс был доступен и локально (compose dev), и из приватной сети, и из прод-deploy на той же машине через loopback. Скачанные веса в MLX-формате лежат в `~/.cache/huggingface` (унифицировано с другими HF-моделями) — не пересекаются с `~/.ollama/models` (GGUF).

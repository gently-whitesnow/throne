...
Accepted (amended девять раз: (1) убраны `*_from_interview` write-tools, введён отдельный `add_intent_qa`; (2) убран `reason?` из edit-tools, тип возврата write-tools обновлён под доменную модель ADR-0002 после её аменда — без `qa[]`/`review[]`; (3) `text_versions` перешёл на delta-формат — описание записи в `replace`/`insert` обновлено под ADR-0002 §4; (4) tool surface сужен до 12 штук — выкинуты `list_*`, `get_instruction(id)`, `read_instruction_text` / `search_instruction_text` и `include_text?` флаг как избыточные для MVP-flows из [readme.md](../../readme.md); (5) write-tools для Instruction убраны — инструкции в MVP редактируются пользователем напрямую (mongosh / будущий HTTP-эндпойнт), агент только читает через bundle; (6) для UI введены HTTP read-эндпойнты `GET /api/v1/instructions` и `GET /api/v1/instructions/{id}` (плюс симметричный `GET /api/v1/intents/{id}`) — read-only surface для фронта `apps/web`; (7) для UI введён write-surface через HTTP: `POST /api/v1/intents`, `POST /api/v1/intents/{id}/replace-text`, `DELETE /api/v1/intents/{id}` (каскадно удаляет `text_versions` агрегата), `POST /api/v1/instructions/{id}/replace-text`, плюс `GET /{id}/versions` для обоих агрегатов. `changed_by=user` для всех HTTP-write вызовов, MCP-агент по-прежнему пишется как `agent`. Write через MCP для Instruction **по-прежнему запрещён** — пользователь правит через UI, агент только читает; (8) §8.1 — MCP wire-policy: prompt-like text tools отдают полный текст только через `Content[]` (StructuredContent — compact refs без `text`/`patch_text`/...), structured-data tools остаются на `UseStructuredContent`. Write-tools `create_intent` / `replace_intent_text` / `insert_intent_text_after_line` возвращают compact ack. Закрывает регрессию intent `6e96cd22…` после `9cc71a8c…`; (9) §8.1 — wire-policy сужена: для всех 9 prompt-like read-tools `StructuredContent = null` **обязательно** (не «либо null, либо compact refs»). Compact refs живут в audit-канале через OOB envelope `McpToolPayload`, не в wire. Закрывает остаточную регрессию intent `ef25f66c…` после `6e96cd22…`: эмпирически любой непустой StructuredContent перехватывает model-facing рендеринг у Claude Code stdio MCP.)

**Update 2026-06-21:** operational MCP tools are retired by [ADR-0043](0043-static-operational-skills-and-mcp-removal.md). `Intent.text` agent operations now run through `skills/intent/bin/throne-intent` over HTTP; the historical MCP tool semantics below are superseded for runtime.
...
#### 8.1. MCP wire-policy: Content vs StructuredContent

MCP spec 2025-11-25 трактует `content[]` как required и primary канал tool_result, а `structuredContent` — как опциональный машинный side-channel. Эмпирически (Claude Code stdio MCP, claude.ai remote-MCP relay): structured-aware клиент **любой непустой** `structuredContent` интерпретирует как model-facing payload и прячет `content[]` от модели. Промежуточная политика «compact refs в StructuredContent» из первой итерации §8.1 эту проблему **не закрыла** — incident intent `ef25f66c…` после `6e96cd22…`. Дублировать полный prompt-like текст в обе ветки тоже нельзя — на кириллице ответ растёт ×2 после wire-encoding и упирается в client-side лимиты `tool_result`.

Универсальное правило, применимое ко всем MCP-tools Throne:

1. **Prompt-like text tools** (результат — model-consumable текст: bundle инструкций, `Intent.text`, slice'ы, content patches): полный текст уходит **только** через `Content[]` — один `TextContentBlock` с понятными разделителями. `UseStructuredContent` для таких tool-ов **выключен**.
2. Для prompt-like tools `CallToolResult.StructuredContent` **обязательно** `null` (не «либо null, либо compact refs»). Любой непустой StructuredContent перехватывает model-facing рендеринг — это контрактное требование к wire-форме, а не оптимизация. Compact refs (ids, versions, counters, link/attachment manifests) живут **только** в audit-канале (`mcp_call_log.result_summary`), не в wire.
3. Передача refs в audit идёт через OOB envelope `McpToolPayload(CallToolResult Wire, IReadOnlyDictionary<string, object?>? AuditSummary)`. Renderer возвращает envelope; `AuditingMcpServerTool.InvokeAsync` распаковывает: `Wire` уходит по wire (с `StructuredContent = null`), `AuditSummary` идёт прямо в `McpAuditLogger` через `overrideSummary` параметр. `McpToolResultConverter` имеет защитный case для `McpToolPayload`, не синтезирующий auto-StructuredContent. Аудит prompt-like tool-ов **не** десериализует wire — refs формируются доменным кодом в момент построения envelope.
4. **Structured-data tools** (link / list / status / move / tags / dream / write-ack) остаются на `UseStructuredContent = true` — там нет больших текстов, и Zod-валидаторы клиентов корректно читают типизированный StructuredContent. Их audit-summary по-прежнему собирается через `McpResultSummarizer.Summarize(callResult)` из wire StructuredContent (fallback ветка `McpAuditResolution.ResolveSummary` при `overrideSummary is null`).
5. **Write tools для Intent** (`create_intent`, `replace_intent_text`, `insert_intent_text_after_line`) возвращают compact `Ack { intent_id, current_version, accepted }`; если агенту нужен post-write текст — он перечитывает через `get_intent` / `read_intent_text`.
6. Полный список 9 prompt-like tools, на которые распространяется правило: `get_intent`, `get_instruction_bundle`, `read_intent_text`, `read_intent_attachment_text`, `search_intent_text`, `get_current_instruction`, `get_instruction_patch`, `list_instruction_patches`, `propose_instruction_patch`.

Регрессионные гейты:
- `Throne.Api.Tests/Mcp/McpToolWireShapeTests` проверяет per-renderer, что wire `StructuredContent is null` и в `AuditSummary` нет запрещённых полей (`text` / `content` / `patch_text` / `applied_text` / `current_instruction_text` / `base_instruction_text` / `rationale` / `note`).
- `Throne.Api.Tests/Mcp/AuditingMcpServerToolTests` дополнительно проверяет end-to-end через `AuditingMcpServerTool`: после распаковки envelope wire `StructuredContent` остаётся `null`, а `mcp_call_log.result_summary` получает refs из OOB канала.
- Wire-size benchmark на синтетическом bundle `system+user × common+work` фиксирует, что end-to-end ответ помещается в 30 KB — это закрывает класс проблем с client-side лимитами `tool_result` при больших инструкциях.

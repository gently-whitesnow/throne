...
- ~~**Никакого хука «status → kill tmux»**. Переходы интента в `done`/`reject`/`fridge` не убивают живую tmux-сессию.~~ **Частично пересмотрено — см. § 8 ниже.** Закрытие интента (`done` или `reject`) теперь убивает сессию; `fridge` (заморозка) по-прежнему её не трогает.
...
### 8. Хук «закрытие интента (`done`/`reject`) ⇒ kill tmux» (пересматривает § 7)

> **Амендмент (2026-06-23):** триггер расширен с `done` на закрытие интента — `done` **или** `reject`. Отклонённый интент так же убивает сессию и сносит локальное состояние (раньше `reject` оставлял живую сессию и мусор в workspace, который оператор убирал руками). `fridge` (заморозка «вернёмся позже») по-прежнему не трогает ни сессию, ни папку. Внутренние C#-символы переименованы в нейтральное `...OnClose` (close = `done`|`reject`); wire-имена (REST-поле `cleanup_local_state_on_done`, Mongo-поле, web-слайс) оставлены без изменений по KISS, чтобы не ломать контракт и не мигрировать Mongo.

Закрытие интента — переход в `done` или `reject`, и авто-по-мерджу PR, и ручной из UI — убивает tmux-сессию интента: `tmux kill-session -t throne-{intent_id}`, идемпотентно (нет сессии → молча ок). Реализовано доменным обработчиком на событие `IntentStatusChanged` (статус `done`|`reject`), переиспользующим тот же kill-путь, что и restart-эндпоинт (§ 2). `tmux list-clients` не проверяется, даже если кто-то приаттачен.

Kill **гейтится** флагом `IntentState.CleanupLocalStateOnClose` (default `true`) — тем же, что и снос рабочей папки + trust-записей. Раньше kill был безусловным, а «оставить состояние» работало лишь побочно, не давая интенту дойти до `done`; любой другой путь в закрытие убивал сессию вопреки желанию оператора. Теперь «остановка терминала» и «снос локального состояния» — две половины одного решения «teardown при закрытии»: снятый флаг сохраняет и сессию, и файлы за закрытием, независимо от пути в него. Снимается/ставится флаг на самом интенте; галка на ревью пишет тот же флаг (D1) и отдельно гейтит авто-закрытие на мерже (D2, `SuppressMergeAutoClose`, ADR-0024).

Почему пересматриваем § 7 «никакого хука status → kill»:

- После закрытия (`done`/`reject`, с включённым флагом) агентская сессия не нужна; ручное закрытие — явный сигнал «работа закончена / отклонена».
...
Границы решения сохраняются: `fridge` (заморозка) сессию и папку **не** трогает — закрывают только `done`/`reject`; `Intent`-схема не расширяется; ограничение «никакого хука» для остальных нетерминальных статусов остаётся в силе.

...
{
    public Task<ActionResult<IntentAttachmentDto>> RunAsync(
        string id,
        HttpRequest request,
    ...
    )
    {
        ...
        {
            return Task.FromResult<ActionResult<IntentAttachmentDto>>(
    ...
    private async Task<ActionResult<IntentAttachmentDto>> UploadCoreAsync(
        string id,
        IFormFile formFile,
        CancellationToken cancellationToken
    )
    {
        ...
        var location =
            $"/api/v1/intents/{Uri.EscapeDataString(id)}/attachments/{Uri.EscapeDataString(attachment.Id)}";

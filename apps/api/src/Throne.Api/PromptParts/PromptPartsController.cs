    ...
    ListPromptPartVersionsHandler listVersionsHandler) : PromptPartsControllerBase
{
    ...
    public override async Task<ActionResult<PromptPartDto>> CreatePromptPart(CreatePromptPartRequest body)
    {
    ...
    public override async Task<ActionResult<PromptPartDto>> GetPromptPart(string id)
    {
        try
        {
            var part = await getHandler.HandleAsync(new GetPromptPartQuery(id), HttpContext.RequestAborted);
            return Ok(PromptPartDtoMapper.ToDetailDto(part));
        }
        catch (ApiException ex)
        {
    ...
    public override async Task<ActionResult<PromptPartDto>> ReplacePromptPartText(string id, ReplaceTextRequest body)
    {
        ArgumentNullException.ThrowIfNull(body);
        try
        {
            var part = await replaceHandler.HandleAsync(
                new ReplacePromptPartTextCommand(id, body.Expected_version, body.Old_text, body.New_text),
                HttpContext.RequestAborted);
            return Ok(PromptPartDtoMapper.ToDetailDto(part));
        }
        catch (ApiException ex)
    ...
    public override async Task<ActionResult<PromptPartDto>> SetPromptPartRoles(string id, SetPromptPartRolesRequest body)
    {
        ArgumentNullException.ThrowIfNull(body);
        try
        {

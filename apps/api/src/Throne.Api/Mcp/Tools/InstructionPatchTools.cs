...
///
/// Wire-policy (ADR-0003 §8): every textual body (patch_text / applied_text /
/// current_instruction_text / base_instruction_text / rationale / reject_comment)
/// goes only into <see cref="TextContentBlock.Text"/>; StructuredContent carries
/// compact refs (id, kind, status, versions, timestamps, evidence ids).
/// </summary>
[McpServerToolType]
    ...
    GetCurrentInstructionHandler currentHandler)
{
        ...
        CancellationToken cancellationToken = default)
    {
        var patch = await proposeHandler.HandleAsync(
            new ProposeInstructionPatchCommand(
                ...
                idempotency_key),
            cancellationToken);
        return InstructionPatchRenderer.RenderProposed(InstructionPatchMcpMapper.ToReadModel(patch));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var page = await listHandler.HandleAsync(
                ...
                cancellationToken);
            var items = page.Items.Select(InstructionPatchMcpMapper.ToReadModel).ToList();
            return InstructionPatchRenderer.RenderList(items, page.NextCursor);
        }
    ...
    [McpServerTool(Name = "get_instruction_patch", ReadOnly = true)]
    ...
    public async Task<McpToolPayload> GetInstructionPatch(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return InstructionPatchRenderer.RenderDetail(InstructionPatchMcpMapper.ToReadModel(view.Patch), view);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return CurrentInstructionRenderer.Render(view);
    }
}

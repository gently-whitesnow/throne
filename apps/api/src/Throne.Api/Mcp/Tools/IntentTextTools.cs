...
{
    [McpServerTool(Name = "read_intent_text", ReadOnly = true)]
    ...
    public async Task<McpToolPayload> ReadIntentText(
    ...
    {
        var slice = await read.HandleAsync(new ReadIntentTextQuery(intent_id, start_line, line_count, max_chars), cancellationToken);
        return TextSliceRenderer.Render(slice);
    }

    [McpServerTool(Name = "replace_intent_text", UseStructuredContent = true)]
    [Description("Replace one unique byte-exact substring in Intent.text using optimistic concurrency. Whitespace is literal: NBSP (U+00A0), zero-width characters and line endings must match exactly. If matching fails, call search_intent_text first and use neighbouring context from its result. Returns a compact ack; re-read with get_intent / read_intent_text if the post-write body is needed.")]
    public async Task<McpWriteAck> ReplaceIntentText(
    ...
    {
        var intent = await replace.HandleAsync(
            new ReplaceIntentTextCommand(intent_id, expected_version, old_text, new_text, TextVersionAuthor.Agent),
            cancellationToken);
    ...
    [Description("Insert text after a line in Intent.text using optimistic concurrency. Use after_line=0 to prepend and after_line=total_lines to append. Returns a compact ack; re-read with get_intent / read_intent_text if the post-write body is needed.")]
    public async Task<McpWriteAck> InsertIntentTextAfterLine(
        [Description("Intent id to mutate.")] string intent_id,
        [Description("current_version observed from the latest get_intent, read_intent_text, or write result.")] int expected_version,
        ...
        CancellationToken cancellationToken)
    {
        var intent = await insertAfterLine.HandleAsync(
            new InsertIntentTextAfterLineCommand(intent_id, expected_version, after_line, insert_text),
            cancellationToken);
        return new McpWriteAck(intent.Id.Value, intent.State.CurrentVersion, Accepted: true);
    }

    [McpServerTool(Name = "search_intent_text", ReadOnly = true)]
    ...
    public async Task<McpToolPayload> SearchIntentText(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await search.HandleAsync(new SearchIntentTextQuery(intent_id, query, context_lines, limit), cancellationToken);
        return TextSearchResultRenderer.Render(result);
    }
}

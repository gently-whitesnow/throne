    ...
    ListRepositoriesHandler listRepositories)
{
    ...
    [Description("Create or update a titled markdown knowledge page anchored to a repository (provider/owner/repo). The only write tool on this MCP surface. The page slug is unique per repository; pass `expected_version` equal to the current version (from get_repository_document) to update — omit it (or 0) to create the first version. On a version mismatch the call fails with repository_artifact.version_conflict carrying current_version; re-read and retry. render_hint is NOT a parameter — it is derived from the slug convention (`db-schema-map` renders as a schema map, every other slug as plain markdown). Pages are deliberately invisible to get_intent and are never fed into an agent's intent context.")]
    public async Task<McpRepositoryDocumentRef> WriteRepositoryDocument(
        ...
        [Description("Stable page slug, unique per repository. Use `db-schema-map` for the DB schema map.")] string slug,
    ...
    [Description("Read a repository knowledge page by (provider, owner, repo, slug). Returns title, document, render_hint and version — take `version` as the next write_repository_document `expected_version`. Fails with repository_artifact.not_found when the page does not exist yet.")]
    public async Task<McpRepositoryDocument> GetRepositoryDocument(
        ...
        [Description("Page slug to read (e.g. db-schema-map).")] string slug,
        CancellationToken cancellationToken = default)
    {
        var coordinate = new RepoCoordinate(provider, owner, repo);
    ...
    [Description("List every repository the server knows (registered from a bind, a tag default, or a knowledge-page write), each with the slugs of pages already written to it. A session that has no intent uses this to confirm its current clone's `git remote` coordinate is registered and to see whether `db-schema-map` already exists before authoring it. Empty `items` just means nothing is registered yet — not an error.")]
...
public sealed record McpRepositoryDocumentRef(
    ...
    [property: Description("Render hint derived from the slug: schema_map or markdown.")] string RenderHint,
    [property: Description("Version after this write; pass as expected_version on the next update.")] int Version)
{
    public static McpRepositoryDocumentRef From(RepositoryArtifact a) =>
        new(a.Slug, a.Title, a.RenderHint, a.Version);
}
...
public sealed record McpRepositoryDocument(
    ...
    [property: Description("Render hint: schema_map or markdown.")] string RenderHint,
    [property: Description("Current version; pass as expected_version on the next update.")] int Version)
{
    public static McpRepositoryDocument From(RepositoryArtifact a) =>
        new(a.Slug, a.Title, a.Document, a.RenderHint, a.Version);
}

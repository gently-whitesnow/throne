...
/// <c>projects["&lt;path&gt;"]</c> trust and project-onboarding flags while preserving every other
...
internal static class ClaudeTrustDocument
{
    ...
    private const string ProjectOnboardingKey = "hasCompletedProjectOnboarding";
    private const string ProjectOnboardingSeenCountKey = "projectOnboardingSeenCount";
    ...
    /// <summary>
    /// Returns the updated document text when <paramref name="workspacePath"/> needed trust or
    /// project-onboarding seeding, or <c>null</c> when no write is warranted: already seeded, or the existing
    ...
    public static string? WithTrustedWorkspace(string? existingJson, string workspacePath)
    {
        ...
        {
            if (HasSeededProjectState(existingEntry))
            {
                return null;
            }
            SeedProjectState(existingEntry);
        }
        else if (projects.ContainsKey(workspacePath))
        {
            return null;
        }
        else
        {
            projects[workspacePath] = SeedProjectState([]);
        }
    ...
    private static bool HasSeededProjectState(JsonObject entry) =>
        IsTrueFlag(entry[TrustKey])
        && IsTrueFlag(entry[ProjectOnboardingKey])
        && IsPositiveNumber(entry[ProjectOnboardingSeenCountKey]);

    private static JsonObject SeedProjectState(JsonObject entry)
    {
        entry[TrustKey] = true;
        entry[ProjectOnboardingKey] = true;
        if (!IsPositiveNumber(entry[ProjectOnboardingSeenCountKey]))
        {
            entry[ProjectOnboardingSeenCountKey] = 1;
        }
        return entry;
    }

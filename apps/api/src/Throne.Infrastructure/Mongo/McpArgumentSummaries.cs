...
internal static class McpArgumentSummaries
{
    ...
    public static IReadOnlyDictionary<string, object?>? Build(string toolName, BsonDocument arguments) =>
        toolName switch
        {
            ...
            "add_intent_review" => new Dictionary<string, object?>(StringComparer.Ordinal)
            {
                ["expected_version"] = TryInt(arguments, "expected_version"),
                ["note_preview"] = Preview(arguments, "note"),
            },
            "add_intent_qa" => new Dictionary<string, object?>(StringComparer.Ordinal)
            {
                ["expected_version"] = TryInt(arguments, "expected_version"),
                ["question_preview"] = Preview(arguments, "question"),
            },
            ...
            "propose_dream_rule" => new Dictionary<string, object?>(StringComparer.Ordinal)
            {
                ["run_id"] = TryString(arguments, "run_id"),
                ["target_kind"] = TryString(arguments, "target_kind"),
                ["severity"] = TryString(arguments, "severity"),
                ["proposed_rule_preview"] = Preview(arguments, "proposed_rule"),
            },
            _ => null,
        };

...
public sealed class Intent
{
        ...
        string? sortKey = null)
    {
        ...
        return new Intent(id, ownerUserId, text, IntentStatusNames.Draft, currentVersion: 1, normalized, resolvedSortKey, now, now);
    }
        ...
        string? sortKey = null)
    {
        ...
        var resolvedSortKey = sortKey ?? FractionalIndex.Initial();
        FractionalIndex.ValidateKey(resolvedSortKey, nameof(sortKey));
        return new Intent(id, ownerUserId, text, status, currentVersion, tagIds, resolvedSortKey, createdAt, updatedAt);
    }

    /// <summary>
    /// Reorder intent by assigning a new sort key. Does not bump <see cref="CurrentVersion"/>
    /// nor <see cref="UpdatedAt"/> — drag-and-drop is purely positional and must not pollute
    /// the text-edit history (see Intent b6abf6 / sortKey ADR).
    /// </summary>
    public bool MoveTo(string newSortKey)
    {
        FractionalIndex.ValidateKey(newSortKey, nameof(newSortKey));
        if (string.Equals(SortKey, newSortKey, StringComparison.Ordinal))
        {
            return false;
        }
        SortKey = newSortKey;

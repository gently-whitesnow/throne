...
public class IntentReviewTests
{
    ...
    public void Create_populates_fields()
    {
        ...
        var review = IntentReview.Create("rev-1", intentId, intentVersionAtWrite: 4,
    ...
    public void Create_validates_inputs()
    {
        ...
        Action emptyN = () => IntentReview.Create("id", intentId, 1, "", "r", Now, IntentTrainingAuthor.Agent);
        emptyN.Should().Throw<ArgumentException>().WithParameterName("note");

        Action emptyR = () => IntentReview.Create("id", intentId, 1, "n", "", Now, IntentTrainingAuthor.Agent);
        emptyR.Should().Throw<ArgumentException>().WithParameterName("reason");

        Action zeroVersion = () => IntentReview.Create("id", intentId, 0, "n", "r", Now, IntentTrainingAuthor.Agent);

...
public class TransactionRollbackTests(MongoFixture fixture)
{
    ...
    public async Task Commits_both_writes_on_success()
    {
        ...
        (await db.GetCollection<TextVersionDocument>(MongoCollectionNames.TextVersions)
            .Find(x => x.OwnerId == id.Value).AnyAsync()).Should().BeTrue();
    }
    ...
    public async Task Rolls_back_first_write_on_failure()
    {
        ...
        (await db.GetCollection<TextVersionDocument>(MongoCollectionNames.TextVersions)
            .Find(x => x.OwnerId == id.Value).AnyAsync()).Should().BeFalse();
    }

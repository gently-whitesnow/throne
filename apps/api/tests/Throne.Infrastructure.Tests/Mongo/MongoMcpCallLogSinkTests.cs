...
public class MongoMcpCallLogSinkTests(MongoFixture fixture)
{
    ...
    public async Task WriteAsync_persists_entry()
    {
        ...
        doc.ExceptionType.Should().BeNull();
    }

    [Fact(DisplayName = "WriteAsync сохраняет error_message и exception_type для error-исхода")]
    public async Task WriteAsync_persists_error_fields()
    {
        var dbName = $"throne_audit_{Guid.NewGuid():N}";
        await fixture.Client.DropDatabaseAsync(dbName);
        var db = fixture.Client.GetDatabase(dbName);
        ...
        var entry = new McpCallLogEntry(
            ...
            SessionId: "session-1",
            UserId: "user-42",
            ToolName: "replace_intent_text",
            Arguments: new Dictionary<string, object?> { ["intent_id"] = "abc" },
            IntentId: "abc",
            ModeHint: null,
            Outcome: McpCallOutcome.Error,
            ErrorCode: "internal_error",
            ErrorMessage: "The arguments dictionary is missing a value for the required parameter 'new_text'.",
        ...
        doc.Outcome.Should().Be("error");
        doc.ErrorCode.Should().Be("internal_error");
        doc.ErrorMessage.Should().Be("The arguments dictionary is missing a value for the required parameter 'new_text'.");

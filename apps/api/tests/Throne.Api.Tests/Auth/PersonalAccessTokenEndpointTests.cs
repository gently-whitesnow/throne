...
public sealed class PersonalAccessTokenEndpointTests(MongoFixture mongo) : IAsyncLifetime
{
    ...
    [Fact(DisplayName = "GET /api/v1/dream-sessions без токена под Mode=Jwt возвращает 401 (FallbackPolicy)")]
    public async Task Dream_sessions_without_token_is_401()
    {
        var response = await _client.GetAsync(new Uri("/api/v1/dream-sessions", UriKind.Relative));
        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact(DisplayName = "GET /api/v1/dream-sessions с валидным PAT проходит auth (PAT-middleware на корне)")]
    public async Task Dream_sessions_with_valid_pat_passes_auth()
    {
        var jwt = IssueJwt("user-pat-ds-1");
        var issued = await IssueTokenAsync(jwt);

        using var request = new HttpRequestMessage(HttpMethod.Get, new Uri("/api/v1/dream-sessions", UriKind.Relative));
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", issued.Token);
        var response = await _client.SendAsync(request);
    ...
    [Fact(DisplayName = "GET /health без токена под Mode=Jwt возвращает 200 (anonymous endpoint)")]
    public async Task Health_endpoint_anonymous()
    {

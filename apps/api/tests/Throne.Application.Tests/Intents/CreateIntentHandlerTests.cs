...
public class CreateIntentHandlerTests
{
    ...
    public async Task CreateIntent_persists_intent_and_v1_snapshot()
    {
        ...
        var handler = new CreateIntentHandler(repo, new IntentTagResolver(new TagNameEnsurer(tagRepo), new TagIdLookup(tagRepo)), uow, new TestCurrentUserAccessor(), clock);

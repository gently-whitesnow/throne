...
/**
 * Materialises a board card into a fresh intent: create from its canonical text
 * → attach the card as a read-only snapshot → navigate to the new
...
export function useCreateIntentWithCard(
  ...
  const createWithCard = async (card: TaskTrackerCard): Promise<void> => {
    ...
    // Intent.Create rejects an empty body, so use the canonical card text with an
    // id fallback for a malformed upstream payload.
    const seed =
      card.text.trim().length > 0 ? card.text : `Карточка ${card.card_id}`;

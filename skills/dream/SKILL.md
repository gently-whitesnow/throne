...
2. **Рефлексия по прошлым правкам — ВСЕГДА перед поиском нового.** `patches --status applied`
   ...
   - **Eviction / demote — рефлексия не только аддитивная.** Помимо «прижилось / не прижилось»
     для каждого applied-правила взвесь два хода на УБОРКУ и предложи их наравне с добавлением:
     * правило прижилось НАСТОЛЬКО, что стало дефолтным поведением модели (агент делает это и
       без напоминания в диалогах), ИЛИ устарело после нового ADR / смены контура → предложи
       **ретайр** (`operation=delete`) либо **демоут роли** (`operation=set_roles`,
       `default_on`→`default_off`), если правило ещё ситуативно полезно, но не должно есть
       бюджет ядра по умолчанию;
     * пример: часть `commit` годами держит «не добавляй Co-Authored-By». Если в свежих
       диалогах агент и так этого не делает — кандидат на демоут роли в режимах, где правило
       избыточно. Доложи как находку, оформи `set_roles`/`delete`-патчем (шаг 9).
